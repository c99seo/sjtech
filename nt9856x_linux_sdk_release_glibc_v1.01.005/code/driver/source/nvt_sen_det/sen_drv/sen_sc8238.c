...
// version
//=============================================================================
VOS_MODULE_VERSION(nvt_sen_sc8238, 1, 11, 000, 00);
...
#define SENSOR_ID                  0x8235

//#define SEN_I2C_NAME "sen_sc8238"
...
static void sen_get_flip_sc8238(CTL_SEN_ID id, CTL_SEN_FLIP *flip);
...
static ER sen_get_attr_cmdif_sc8238(CTL_SENDRV_GET_ATTR_CMDIF_PARAM *data);
...
static void sen_ext_get_probe_sen_sc8238(CTL_SEN_ID id, CTL_SENDRV_GET_PROBE_SEN_PARAM *data);
//=============================================================================
// global variable
//=============================================================================
...
static CTL_SEN_CMD sc8238_mode_2[] = {
	...
	{0x394f, 1, {0x90, 0x0}},
	{0x3981, 1, {0x70, 0x0}},
	{0x3984, 1, {0x20, 0x0}},//blc trigger¢G?alpha trigger¢G??a?oblcAs?-£gaAeE?£gaEAE¡MIEIa
	{0x3987, 1, {0x08, 0x0}},
...
static ISP_SENSOR_CTRL sensor_ctrl_last = {0};
static ISP_SENSOR_PRESET_CTRL preset_ctrl[CTL_SEN_ID_MAX] = {0};
static INT32 is_fastboot;
static UINT32 fastboot_i2c_id;
static UINT32 fastboot_i2c_addr;
...
static void sen_pwr_ctrl_sc8238(CTL_SEN_ID id, CTL_SEN_PWR_CTRL_FLAG flag, CTL_SEN_CLK_CB clk_cb)
{
	...
	if ((flag == CTL_SEN_PWR_CTRL_TURN_ON) && ((!is_fastboot) || (fastboot_i2c_id != sen_i2c[id].id) || (fastboot_i2c_addr != sen_i2c[id].addr))) {
		if (clk_cb != NULL) {
...
static ER sen_open_sc8238(CTL_SEN_ID id)
{
	...
	if ((!is_fastboot) || (fastboot_i2c_id != sen_i2c[id].id) || (fastboot_i2c_addr != sen_i2c[id].addr)) {
...
static ER sen_write_reg_sc8238(CTL_SEN_ID id, CTL_SEN_CMD *cmd)
{
	...
	if ((!is_fastboot) || (fastboot_i2c_id != sen_i2c[id].id) || (fastboot_i2c_addr != sen_i2c[id].addr)) {
		...
		#if defined(__KERNEL__)
		isp_builtin_set_transfer_i2c(&msgs, 1);
		#endif
	}
...
static ER sen_read_reg_sc8238(CTL_SEN_ID id, CTL_SEN_CMD *cmd)
{
	...
	if ((!is_fastboot) || (fastboot_i2c_id != sen_i2c[id].id) || (fastboot_i2c_addr != sen_i2c[id].addr)) {
		...
		#if defined(__KERNEL__)
		isp_builtin_set_transfer_i2c(msgs, 2);
		#endif
	}
		...
		if (p_cmd_list[idx].addr == SEN_CMD_DELAY) {
			...
			switch (preset_ctrl[id].mode) {
				...
				case ISP_SENSOR_PRESET_CHGMODE:
					memcpy(&sensor_ctrl, &sensor_ctrl_last, sizeof(ISP_SENSOR_CTRL));
					break;
...
static ER sen_get_info_sc8238(CTL_SEN_ID id, CTL_SENDRV_CFGID drv_cfg_id, void *data)
{
	...
	case CTL_SENDRV_CFGID_GET_ATTR_CMDIF:
		rt = sen_get_attr_cmdif_sc8238((CTL_SENDRV_GET_ATTR_CMDIF_PARAM *)(data));
		break;
	...
	case CTL_SENDRV_CFGID_GET_MODE_ROWTIME:
		...
		break;	
	case CTL_SENDRV_CFGID_FLIP_TYPE:
		sen_get_flip_sc8238(id, (CTL_SEN_FLIP *)(data));
		break;
    case CTL_SENDRV_CFGID_GET_PROBE_SEN:
        sen_ext_get_probe_sen_sc8238(id, (CTL_SENDRV_GET_PROBE_SEN_PARAM *)(data));
        break;
...
static void sen_set_gain_sc8238(CTL_SEN_ID id, void *param)
{
	...
	sensor_ctrl_last.gain_ratio[0] = sensor_ctrl->gain_ratio[0];
	sensor_ctrl_last.gain_ratio[1] = sensor_ctrl->gain_ratio[1];
...
static void sen_set_expt_sc8238(CTL_SEN_ID id, void *param)
{
	...
	sensor_ctrl_last.exp_time[0] = sensor_ctrl->exp_time[0];
	sensor_ctrl_last.exp_time[1] = sensor_ctrl->exp_time[1];
	...
	//Check max exp line
	if ((line[0] * 2) > ((sensor_vd * 2) - NON_EXPOSURE_LINE)) {
		line[0] = ((sensor_vd * 2) - NON_EXPOSURE_LINE) / 2;
	}

	// set exposure line to sensor
	cmd = sen_set_cmd_info_sc8238(0x3E00, 1, ((line[0] * 2) >> 12) & 0x0F, 0);
	rt |= sen_write_reg_sc8238(id, &cmd);
	cmd = sen_set_cmd_info_sc8238(0x3E01, 1, ((line[0] * 2) >> 4) & 0xFF , 0);
	rt |= sen_write_reg_sc8238(id, &cmd);
	cmd = sen_set_cmd_info_sc8238(0x3E02, 1, ((line[0] * 2) & 0x0F) << 4, 0);
	rt |= sen_write_reg_sc8238(id, &cmd);
...
#endif
{
	...
	sensor_ctrl->gain_ratio[0] = sensor_ctrl_last.gain_ratio[0];
	sensor_ctrl->gain_ratio[1] = sensor_ctrl_last.gain_ratio[1];
}
...
#endif
{
	...
	sensor_ctrl->exp_time[0] = sensor_ctrl_last.exp_time[0];
	sensor_ctrl->exp_time[1] = sensor_ctrl_last.exp_time[1];
}
...
static void sen_get_min_expt_sc8238(CTL_SEN_ID id, void *param)
{
	...
	t_row = sen_calc_rowtime_sc8238(id, cur_sen_mode[id]);
	*min_exp_time = t_row * MIN_EXPOSURE_LINE / 10; // if linear and shdr mode is different,the value need to separate.
}
...
static void sen_ext_get_probe_sen_sc8238(CTL_SEN_ID id, CTL_SENDRV_GET_PROBE_SEN_PARAM *data)
{
    ...
    cmd = sen_set_cmd_info_sc8238(0x3107, 1, 0x0, 0x0);
    rt |= sen_read_reg_sc8238(id, &cmd);
    ...
    cmd = sen_set_cmd_info_sc8238(0x3108, 1, 0x0, 0x0);
    rt |= sen_read_reg_sc8238(id, &cmd);
...
int sen_init_sc8238(SENSOR_DTSI_INFO *info)
{
	...
	is_fastboot = 0;
	fastboot_i2c_id = 0xFFFFFFFF;
	fastboot_i2c_addr = 0x0;
...
static int __init sen_init_sc8238(void)
{
	...
	fastboot_i2c_id = isp_builtin_get_i2c_id();
	fastboot_i2c_addr = isp_builtin_get_i2c_addr();
