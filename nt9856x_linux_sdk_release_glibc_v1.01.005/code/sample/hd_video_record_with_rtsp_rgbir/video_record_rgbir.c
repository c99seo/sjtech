...
#define RESOLUTION_SET  3 //0: 2M(IMX290), 1:5M(OS05A) 2: 2M (OS02K10) 3: 2M (AR0237IR) 4: 2M (OV2736IR)
#if ( RESOLUTION_SET == 0)
...
static HD_RESULT set_cap_cfg(HD_PATH_ID *p_video_cap_ctrl)
{
	...
	#elif (RESOLUTION_SET == 4)
	snprintf(cap_cfg.sen_cfg.sen_dev.driver_name, HD_VIDEOCAP_SEN_NAME_LEN-1, "nvt_sen_ov2736ir");
	printf("Using nvt_sen_ov2736ir\n");
	#endif
    ...
    if(RESOLUTION_SET == 0 || RESOLUTION_SET == 1 || RESOLUTION_SET == 2 || RESOLUTION_SET == 4){
...
#elif ( RESOLUTION_SET == 4)
	strncpy(cfg_info.path, "/mnt/app/isp/isp_ov2736ir_0.cfg", CFG_NAME_LENGTH);
#endif
...
//
	...
	if (g_prc_out_fmt == 1)
	{
		...
		ret = set_proc_param(stream[0].proc_path, NULL, HD_VIDEO_PXLFMT_YUV420, FALSE);
		if (ret != HD_OK) {
			printf("set proc fail=%d\n", ret);
			goto exit;
		}
