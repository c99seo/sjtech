...
static pthread_t  cap_thread_id;
static UINT32 cap_exit = 0;

#define FAILED_RETRY_CNT 3
static void *cap_thread(void *arg)
{
	VIDEO_LIVEVIEW *p_stream0 = (VIDEO_LIVEVIEW *)arg;
	UINT32 failed_count = 0;
	UINT32 last_vd_count = 0;
	HD_VIDEOCAP_SYSINFO sys_info = {0};

	//--------- pull data test ---------
	while (cap_exit == 0) {
		usleep(1000000);
		hd_videocap_get(p_stream0->cap_ctrl, HD_VIDEOCAP_PARAM_SYSINFO, &sys_info);
		...
		if (sys_info.vd_count <= last_vd_count) {
			failed_count++;
		} else {
			failed_count = 0;
		}
		last_vd_count = sys_info.vd_count;

		if (failed_count >= FAILED_RETRY_CNT) {
			printf("\r\n############### RESET SENSOR ##############\r\n");
			last_vd_count = 0;
			hd_videocap_stop(p_stream0->cap_path);
			hd_videocap_close(p_stream0->cap_path);
			set_cap_cfg(&p_stream0->cap_path);
			hd_videocap_open(HD_VIDEOCAP_0_IN_0, HD_VIDEOCAP_0_OUT_0, &p_stream0->cap_path);
...
EXAMFUNC_ENTRY(hd_video_liveview, argc, argv)
{
	HD_RESULT ret;
	INT key;
	...
	ret = pthread_create(&cap_thread_id, NULL, cap_thread, (void *)stream);
	if (ret < 0) {
