...
int get_struct_size(unsigned int type, int *struct_len)
{
	...
	case FTG:
		*struct_len += sizeof(face_tracking_result) + 4;
		break;
	case CDD:
		*struct_len += sizeof(cdd_result) + 4;
		break;
	default:
...
int copy_result_data(unsigned int type, unsigned char *data, int *len)
{
	...
	case FTG:
		...
		*len += sizeof(face_tracking_result);
		break;
	case CDD:
		memcpy(data + *len, (unsigned char *)gcdd_result, sizeof(cdd_result));
		*len += sizeof(face_result);
		break;
	default:
...
static void init_cdd_detection(void)
{
	gcdd_result = (cdd_result *)malloc(sizeof(cdd_result));
	if(gcdd_result == NULL)
	{
		printf("[%s] gcdd_result is NULL\n",__func__);
		return;
	}
	memset(gcdd_result, 0xff, sizeof(people_result));
	/* VLC do not define CDD. Here just use VEHI to verify
	 *
	 */
	gcdd_result->identity[0] = 0x56; //'V'
	gcdd_result->identity[1] = 0x45; //'E'
	gcdd_result->identity[2] = 0x48; //'H'
	gcdd_result->identity[3] = 0x49; //'I'

}

void generate_cddt_idx(int index,unsigned int id, unsigned int x, unsigned int y, unsigned int w,unsigned int h)
{
	//printf("index = %d,id=%d,x=%x,y=%d,w=%d,h=%d\n",index,id,x,y,w,h);
	gcdd_result->cdd[index].id = id;
    gcdd_result->cdd[index].x = x;
    gcdd_result->cdd[index].y = y;
    gcdd_result->cdd[index].w = w;
    gcdd_result->cdd[index].h = h;
}

//void generate_pdt(int num, unsigned int timestamp,unsigned int x_off,unsigned int y_off)
void generate_cddt(int num, short delay_fps,short trace_fps,unsigned int x_off,unsigned int y_off)
{
	if(gcdd_result == NULL)
	{
        printf("[%s] gcdd_result is NULL\n",__func__);
	...
	//gcdd_result->timestamp = timestamp;
	gcdd_result->trace_fps = trace_fps;
	gcdd_result->delay_fps = delay_fps;
	if(num > MAX_DETECT_COUNT)
		num = MAX_DETECT_COUNT;
	gcdd_result->cdd_num = num;
	return ;
}
