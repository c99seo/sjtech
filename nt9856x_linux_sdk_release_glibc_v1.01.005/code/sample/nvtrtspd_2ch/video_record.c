...
static UINT32 vir_addr_main = 0;
static HD_VIDEOENC_BUFINFO phy_buf_main = {0};
static UINT32 vir_addr_sub = 0;
static HD_VIDEOENC_BUFINFO phy_buf_sub = {0};
...
static HD_RESULT mem_init(void)
{
	...
	//only for shdr + direct mode
	if (g_shdr_mode && g_DATA_MODE) {
		mem_cfg.pool_info[2].type = HD_COMMON_MEM_COMMON_POOL;
		mem_cfg.pool_info[2].blk_size = VDO_NRX_BUFSIZE(VDO_SIZE_W, VDO_SIZE_H)/2;
...
MAIN(argc, argv)
{
	...
	ret = set_enc_cfg(stream[0].enc_path, &stream[0].enc_max_dim, 2 * 1024 * 1024, 0);
	if (ret != HD_OK) {
	...
	// create encode_thread (pull_out bitstream)
	ret = pthread_create(&stream[0].enc_thread_id, NULL, encode_thread, (void *)video_info);
	if (ret < 0) {
