...
void cmd_mem_analysis(int mode, CHAR* filename)
{
	int ret = 0;
	char buf[100] = {0};
	...
	/* ON */
	if (mode == 0) {
		system("echo cma:* kmem:* > /sys/kernel/debug/tracing/set_event");

		ret = sysfs_write("/sys/kernel/debug/tracing/tracing_on", "0");
		if (ret < 0)
			return;

		ret = sysfs_write("/sys/kernel/debug/tracing/trace", " ");
		if (ret < 0)
			return;

		ret = sysfs_write("/sys/kernel/debug/tracing/tracing_on", "1");
		if (ret < 0)
			return;
		DBG_DUMP("Tracing event mode is on\n");
	/* OFF */
	} else if (mode == 1) {
		ret = sysfs_write("/sys/kernel/debug/tracing/tracing_on", "0");
		if (ret < 0)
			return;
		DBG_DUMP("Tracing event mode is off\n");
	/* DUMP */
	} else {
		sprintf(buf, "cat /sys/kernel/debug/tracing/trace > %s", filename);
		system(buf);
		sync();
		DBG_DUMP("Dump file name path: %s\n", filename);
	}
}
...
void show_help(void)
{
	...
	DBG_DUMP("\tanalysis [0/1/2: on/off/dump] (filename)   To analyze system memory allocated result\r\n");
	DBG_DUMP("\te.g.\r\n");
	...
	if (argc <= 1) {
		...
		while (i < argc) {
			...
			if (!strcmp(argv[i], "analysis")) {
				UINT32  mode = 0;
				CHAR	filename[256]="/mnt/sd/mem.log";

				if (argc != 4)
				{
					DBG_DUMP("syntex: mem analysis [0/1/2: on/off/dump] [filename] %d \r\n", argc);
					break;
				}
				i++;
				if (i < argc)
					sscanf(argv[i++], "%lx", &mode);
				if (i < argc)
					sscanf(argv[i++], "%s", filename);
				cmd_mem_analysis(mode, filename);
				break;
			}
			show_help();
			break;
